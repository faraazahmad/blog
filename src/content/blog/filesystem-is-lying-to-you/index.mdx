...
import FsAnim from './FsAnim.astro'
...
<MarginNote
  title=""
>
  CuttleFS runs in userspace via FUSE, so its writeback path differs from a kernel
  filesystem like ext4: it can model fsync() reporting success even though the underlying
  storage write failed. The experiments are therefore *illustrative* of deferred-writeback-error semantics, not a literal ext4 reproduction holds across real filesystems, as the PostgreSQL  fsyncgate saga demonstrated.
</MarginNote>
