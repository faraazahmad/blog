...
And yet it can be the process that observes the writeback error. **This is where 
the mental model really falls apart.** We tend to imagine:
...
And sorry about the clickbait title, it's just what you gotta do in the attention economy.
...
The filesystem is allowed to do asynchronous writeback. Linux has explicit machinery for tracking 
writeback errors precisely because those errors can otherwise disappear into the void.

The weird part is the gap between:

```text
what the system call actually promises
```
...
This is why PostgreSQL gets extremely uncomfortable when it sees an `fsync()` failure.
...
The PostgreSQL response is delightfully conservative:

**If it can't trust the durability boundary, it PANICs.**

At first this sounds ridiculous. Why crash an entire database because one `fsync()` failed?
...
That's not a particularly comforting database.

Imagine the database is now in a state where:
...
While we're talking about things filesystems don't promise, there's another classic one worth 
remembering. Say you want to replace a config file without ever leaving it half-written. The usual pattern is:
...
Why the second `fsync()`? **Because the file contents and the directory entry are different pieces of state.**

First you make the bytes durable:
...
Then you make the **directory change** durable:

```text
fsync(parent directory)
```

The first time I learned this, I remember thinking: _"Wait. The filename needs an `fsync()` too?"_

And the filesystem, with the patience of an extremely literal coworker, basically replies: 
_"You asked me to persist the contents. You did not ask me to persist the name."_

Well fair enough, you live you learn.
...
If you are about to do something that assumes data will survive a crash, you need a real synchronization point.

Don't use `fsync()` because it sounds reassuring.

Use it because your correctness model requires durability.
...
This is probably the most important lesson from the experiment.
...
The error may be telling you about earlier writeback.

The filesystem API does not give you the neat transaction-level mapping your application might wish it did.

### 5. Check later error paths too

On Linux, writeback errors can be reported later through operations such as `fsync()`, `fdatasync()`, or `close()`, depending on the path and filesystem.

So the annoying answer is:

**yes, you need to keep checking.**

### 6. If you're implementing a real WAL, maybe don't invent a database from scratch

SQLite is literally a file.

It still has to deal with all of this nonsense.

But somebody else has spent a *very* long time figuring out crash recovery, journaling/WAL behavior, checksums, concurrency and transaction semantics.

That's one of the great gifts of databases.
...
```text
please consider the possibility that my inode has inherited a deferred
writeback error from a different process while the storage controller
is having a bad day
```
...
and then I can send the money.

But I cannot make these two operations magically become one atomic instruction:

```text
persist intent + send money
```

There is always a nasty little window between them.

So a real payment system still needs things like idempotency, retries, reconciliation and recovery logic.
...
And that is where filesystem durability quietly turns into distributed-systems territory.

The moment another machine is involved, `fsync()` can only solve the problems on **your side** of the boundary.
...
After all of this, I've ended up with a much uglier — but more useful — mental model.
...
And a lot of really nasty systems bugs happen when we casually collapse them into one:
...
```text
fsync() == durable transaction
```

What I actually had was:

```text
fsync() == a synchronization boundary
```

And even a trustworthy local durability boundary doesn't make an external side effect atomic.

So, no.

Your filesystem probably isn't lying to you.

It's being annoyingly precise.
